...
template <class T>
inline char* MRFEnergy<T>::Malloc(int bytesNum)
{
...
template <class T>
MRFEnergy<T>::MRFEnergy(GlobalSize Kglobal, ErrorFunction errorFn)
...
template <class T>
typename MRFEnergy<T>::NodeId MRFEnergy<T>::AddNode(LocalSize K, NodeData data)
{
...
template <class T>
void MRFEnergy<T>::AddNodeData(NodeId i, NodeData data)
{
    i->m_D.Add(m_Kglobal, i->m_K, data);
}

template <class T>
void MRFEnergy<T>::AddEdge(NodeId i, NodeId j, EdgeData data)
{
    if (m_isEnergyConstructionCompleted) {
        m_errorFn("Error in AddNode(): graph construction completed - nodes cannot be added");
    }
...
template <class T>
void MRFEnergy<T>::AddRandomMessages(unsigned int random_seed, REAL min_value, REAL max_value)
{
    Node* i;
    MRFEdge* e;
    int k;
...
template <class T>
void MRFEnergy<T>::CompleteGraphConstruction()
{
    Node* i;
    Node* j;
    MRFEdge* e;
    ...
    m_buf = (char*)Malloc(m_vectorMaxSizeInBytes + (m_vectorMaxSizeInBytes > Edge::GetBufSizeInBytes(m_vectorMaxSizeInBytes) ? m_vectorMaxSizeInBytes : Edge::GetBufSizeInBytes(m_vectorMaxSizeInBytes)));
...
template <class T>
int MRFEnergy<T>::Minimize_TRW_S(Options& options, REAL& lowerBound, REAL& energy, REAL* min_marginals)
{
    Node* i;
    Node* j;
    MRFEdge* e;
...
template <class T>
int MRFEnergy<T>::Minimize_BP(Options& options, REAL& energy, REAL* min_marginals)
{
    Node* i;
    Node* j;
    MRFEdge* e;
...
template <class T>
typename T::REAL MRFEnergy<T>::ComputeSolutionAndEnergy()
{
    Node* i;
    Node* j;
    MRFEdge* e;
...
template <class T>
void MRFEnergy<T>::SetMonotonicTrees()
{
    Node* i;
    MRFEdge* e;
